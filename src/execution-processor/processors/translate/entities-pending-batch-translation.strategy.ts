...
export class EntitiesPendingBatchTranslationStrategy extends TranslationStrategyBase {
  ...
  async execute(execution: ExecutionEntity): Promise<any> {
    const entityDataByIndex: EntityData[] =
      execution.payload['entityDataByIndex'] || [];
    const resourceId = Number(execution.payload['resourceId']);
    const targetLanguages = (execution.payload[
      'targetLanguages'
    ] as string[]) || [execution.payload['targetLanguage'] as string] || ['es'];
    const sourceLanguage =
      (execution.payload['sourceLanguage'] as string) || 'en';

    this.logger.log(
      `Processing entities-pending-batch translation for ${entityDataByIndex.length} entities to languages: ${targetLanguages.join(', ')}`,
    );

    const results = this.getBatchResults(execution, 'entities-pending-batch');
    ...
    // If there are more languages to translate, create a follow-up execution
    if (remainingLanguages.length > 0) {
      const textsForTranslation = entityDataByIndex.map((entity) => ({
        text: entity.word,
      }));
