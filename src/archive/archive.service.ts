...
export class ArchiveService {
  ...
  async unarchiveProject(projectId: number): Promise<void> {
    ...
    const buckets = await this.collectByProject(
      projectId,
      /*onlyActive*/ false,
    );
  ...
  private async collectByProject(
    projectId: number,
    onlyActive: boolean,
  ): Promise<CascadeBuckets> {
  ...
  private async collectThreadIdsForProject(
    projectId: number,
  ): Promise<number[]> {
  ...
  private async moveResourceFilesToArchive(
    resources: ResourceEntity[],
  ): Promise<void> {
    for (const r of resources) {
      ...
      try {
        ...
        this.logger.error(
          `Failed to move file to archive for resource ${r.id} (${r.path}): ${err.message}`,
        );
      }
    }
  }

  private async moveResourceFilesFromArchive(
    resources: ResourceEntity[],
  ): Promise<void> {
    for (const r of resources) {
      if (!r.path) continue;
      ...
      try {
        ...
        this.logger.error(
          `Failed to restore file from archive for resource ${r.id} (${r.path}): ${err.message}`,
        );
      }
    }
  }
