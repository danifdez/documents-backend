...
export class DataSourceScheduleService implements OnModuleInit {
  ...
  private async loadSchedules(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Loaded ${sources.filter((s) => s.syncSchedule && s.enabled).length} scheduled data sources`,
      );
    } catch (err) {
  ...
  registerCron(dataSourceId: number, cronExpression: string): void {
    ...
    try {
      const execution = new CronJob(cronExpression, async () => {
        this.logger.log(`Cron triggered sync for data source ${dataSourceId}`);
        try {
          await this.syncService.syncDataSource(dataSourceId);
        } catch (err) {
          this.logger.error(
            `Scheduled sync failed for data source ${dataSourceId}: ${err.message}`,
          );
        }
      });

      this.schedulerRegistry.addCronJob(jobName, execution);
      execution.start();
      this.logger.log(
        `Registered cron for data source ${dataSourceId}: ${cronExpression}`,
      );
    } catch (err) {
      this.logger.error(
        `Invalid cron expression for data source ${dataSourceId}: ${cronExpression}`,
      );
    }
  }
  ...
  unregisterCron(dataSourceId: number): void {
    ...
    try {
      ...
      // Execution doesn't exist, ignore
    }
  }
