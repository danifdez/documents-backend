  ...
  async findOne(id: number): Promise<ResourceEntity | null> {
    try {
      const resource = await this.repo.findOne({
        select: ['id', 'name', 'title', 'path', 'project', 'summary', 'keyPoints', 'keywords', 'originalName', 'publicationDate', 'type', 'license', 'fileSize', 'pages', 'uploadDate', 'url', 'language', 'mimeType', 'confirmationStatus', 'createdAt', 'updatedAt'],
        where: { id },
        relations: ['project'],
      });
