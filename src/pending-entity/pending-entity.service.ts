...
export class PendingEntityService {
  ...
  async findByResourceId(
    resourceId: number,
  ): Promise<(PendingEntityEntity & { isConfirmed?: boolean })[]> {
        ...
        if (pending.language) {
          if ((this.entityService as any).findByNameTypeAndLocale) {
            existingEntity = await (
              this.entityService as any
            ).findByNameTypeAndLocale(
              pending.name,
  ...
  async update(
    ...
    if (dto.translations !== undefined)
      existing.translations = dto.translations;
    if (dto.entityTypeId !== undefined) {
      existing.entityType = dto.entityTypeId
        ? ({ id: dto.entityTypeId } as any)
        : null;
    }
  ...
  private resolveProjectId(resource: ResourceEntity): number {
    return typeof resource.project === 'object'
      ? resource.project.id
      : resource.project;
  }

  private async materializePendingEntity(
    pending: PendingEntityEntity,
    resource: ResourceEntity,
  ): Promise<void> {
    const allAliases = pending.aliases || [];

    // Find existing entity or create new one
    let entity = await this.entityService.findByNameAndType(
      pending.name,
      pending.entityType.id,
    );
    ...
    if (!entity) {
      ...
      if (pending.scope === 'project' && resource.project) {
        await this.entityService.addProjectToEntity(
          entity.id,
          this.resolveProjectId(resource),
        );
      }
    }
  ...
  async confirmEntities(
    ...
    const pendingEntities = allPendingEntities.filter(
      (e) => e.status !== 'merged',
    );
    ...
    for (const pending of pendingEntities) {
      try {
        ...
        errors.push(
          `Failed to confirm entity "${pending.name}": ${error.message}`,
        );
      }
    }

    // Update resource status to 'confirmed' and launch ingest-content execution.
    ...
    if (features.relationships && confirmed > 0) {
      try {
        ...
        errors.push(
          `Failed to create relationship extraction execution: ${error.message}`,
        );
      }
    }
  ...
  async confirmEntity(
    ...
    try {
      ...
      return {
        success: false,
        message: `Failed to confirm entity: ${error.message}`,
      };
    }
  }
  ...
  async mergeEntity(
    ...
    try {
      ...
      if (targetType === 'pending') {
        ...
        return {
          success: true,
          message: 'Entity marked as merged into pending entity',
          pending: saved,
        } as any;
      } else {
        ...
        if (!confirmedEntity) {
          return {
            success: false,
            message: 'Target confirmed entity not found',
          };
        }
        ...
        return {
          success: true,
          message: 'Entity marked as merged into confirmed entity',
          pending: saved,
        } as any;
      }
    } catch (error) {
  ...
  async cancelMerge(
    ...
    try {
      ...
      if (!source)
        return { success: false, message: 'Source pending entity not found' };
      if (
        source.status !== 'merged' ||
        !source.mergedTargetType ||
        !source.mergedTargetId
      ) {
      ...
      if (source.mergedTargetType === 'pending') {
        const target = await this.repository.findOneBy({
          id: source.mergedTargetId,
        });
        if (target && target.aliases) {
          target.aliases = (target.aliases || []).filter(
            (a) =>
              !(
                a.value === aliasToRemove.value &&
                a.scope === aliasToRemove.scope
              ),
          );
          await this.repository.save(target);
        }
      } else if (source.mergedTargetType === 'confirmed') {
        const confirmed = await this.entityService.findOne(
          source.mergedTargetId!,
        );
        if (confirmed && confirmed.aliases) {
          // Confirmed entity alias shape may not include scope; remove by value match
          const filtered = (confirmed.aliases || []).filter(
            (a) => a.value !== aliasToRemove.value,
          );
      ...
      return {
        success: false,
        message: `Failed to cancel merge: ${error.message}`,
      };
    }
  }
  ...
  async batchUpdateTranslations(
    updates: Array<{ id: number; translations: EntityTranslation }>,
  ): Promise<void> {
  ...
  async retranslateEntity(
    ...
    try {
      ...
      if (
        targetLanguage !== 'en' &&
        targetLanguage !== resourceLanguage &&
        !updatedTranslations[targetLanguage]
      ) {
        languagesToTranslate.push(targetLanguage);
      }

      // If there are languages to translate, create a translation execution
      ...
      return {
        success: false,
        message: `Failed to retranslate: ${error.message}`,
      };
    }
  }
}
