...
function createMockRepo() {
  ...
  return {
    ...
    delete: jest.fn(async ({ id, assistantId }: any) => {
      if (id != null) {
        store.delete(id);
        return { affected: 1 };
      }
      if (assistantId != null) {
        for (const [k, v] of store.entries()) {
          if (v.assistantId === assistantId) store.delete(k);
        }
      }
      return { affected: 1 };
    }),
...
describe('IndexedFileService', () => {
  ...
  it('writeFile creates file on disk and row in DB', async () => {
    const entity = await service.writeFile(assistantId, 'notes.md', '# Hello', { overwrite: false });
    expect(entity.filename).toBe('notes.md');
    expect(entity.assistantId).toBe(assistantId);
  ...
  it('writeFile throws ConflictException when file exists without overwrite', async () => {
    await service.writeFile(assistantId, 'lista.md', 'one', { overwrite: false });
    await expect(
      service.writeFile(assistantId, 'lista.md', 'two', { overwrite: false }),
    ).rejects.toThrow(ConflictException);
  });

  it('writeFile with overwrite updates existing row', async () => {
    const first = await service.writeFile(assistantId, 'lista.md', 'one', { overwrite: false });
    const second = await service.writeFile(assistantId, 'lista.md', 'two', { overwrite: true });
  ...
  it('writeFile accepts a Buffer for binary content', async () => {
    // 4-byte PDF-like header + some bytes that are NOT valid UTF-8 to ensure
    // the bytes go to disk verbatim (no implicit string conversion).
    const buf = Buffer.from([0x25, 0x50, 0x44, 0x46, 0xff, 0xfe, 0x00, 0x01]);
    const entity = await service.writeFile(assistantId, 'report.pdf', buf, { overwrite: false });
  ...
  it('writeFile uses MIME from filename, not from content (xlsx case)', async () => {
    // openpyxl-style xlsx starts with PK (zip header). The mime must come
    // from the .xlsx extension via MIME_MAP, not be sniffed from bytes.
    const xlsxLike = Buffer.from([0x50, 0x4b, 0x03, 0x04, 0x00]);
    const entity = await service.writeFile(assistantId, 'data.xlsx', xlsxLike, { overwrite: false });
  ...
  it('deleteFile is idempotent', async () => {
    const e = await service.writeFile(assistantId, 'borrar.md', 'x', { overwrite: false });
    await service.deleteFile(e.id, assistantId);
    await service.deleteFile(e.id, assistantId);
    expect(indexedRepo.store.size).toBe(0);
  });

  it('clearAllForAssistant removes all rows for the assistant', async () => {
    await service.writeFile(assistantId, 'a.md', 'a', { overwrite: false });
    await service.writeFile(assistantId, 'b.md', 'b', { overwrite: false });
    await service.clearAllForAssistant(assistantId);
    expect(indexedRepo.store.size).toBe(0);
  });

  describe('scanFolder', () => {
    it('returns no_folder when assistant has no folderScope', async () => {
      assistantRepo.store.set(assistantId, { id: assistantId, folderScope: null });
      const result = await service.scanFolder(assistantId);
    ...
    it('detects modified files and resets extracted fields', async () => {
      await service.writeFile(assistantId, 'a.md', 'first', { overwrite: false });
    ...
    it('detects removed files', async () => {
      await service.writeFile(assistantId, 'a.md', 'x', { overwrite: false });
    ...
    it('serializes concurrent scans for the same assistant', async () => {
      await fs.writeFile(path.join(tmpScope, 'a.md'), 'A');
      const [r1, r2] = await Promise.all([
        service.scanFolder(assistantId),
        service.scanFolder(assistantId),
      ]);
      expect(r1).toBe(r2);
    });
    ...
    it('enqueues an extraction job per new file', async () => {
      ...
      await service.scanFolder(assistantId);
  ...
  describe('readWithSync', () => {
    it('returns content of an indexed text file', async () => {
      const f = await service.writeFile(assistantId, 'a.md', 'hello', { overwrite: false });
      const result = await service.readWithSync(assistantId, { indexedFileId: f.id });
    ...
    it('detects ambiguous filenames', async () => {
      await service.writeFile(assistantId, 'recipes/paella.md', 'x', { overwrite: false });
      await service.writeFile(assistantId, 'archive/paella.md', 'y', { overwrite: false });
      const result = await service.readWithSync(assistantId, { filename: 'paella.md' });
    ...
    it('reads via basename when only one match exists', async () => {
      await service.writeFile(assistantId, 'recipes/paella.md', 'rice', { overwrite: false });
      const result = await service.readWithSync(assistantId, { filename: 'paella.md' });
      expect(result.ok).toBe(true);
    ...
    it('reindexes when the file changed on disk', async () => {
      const f = await service.writeFile(assistantId, 'a.md', 'first', { overwrite: false });
    ...
    it('adopts a file that exists on disk but not in the index', async () => {
      ...
      const result = await service.readWithSync(assistantId, { filename: 'orphan.md' });
      expect(result.ok).toBe(true);
  ...
  it('writeFile enqueues an extraction job', async () => {
    await service.writeFile(assistantId, 'a.md', 'hello', { overwrite: false });
  ...
  describe('vector cleanup', () => {
    it('deleteFile enqueues an indexed-file-delete-vectors job by sourceId', async () => {
      const e = await service.writeFile(assistantId, 'a.md', 'x', { overwrite: false });
    ...
    it('clearAllForAssistant enqueues cleanup by assistantId', async () => {
      await service.writeFile(assistantId, 'a.md', 'x', { overwrite: false });
      jobService.create.mockClear();
      await service.clearAllForAssistant(assistantId);
      const cleanupCall = jobService.create.mock.calls.find(
      ...
      expect(cleanupCall).toBeDefined();
      expect(cleanupCall![2]).toMatchObject({ assistantId });
    });
  });
});
