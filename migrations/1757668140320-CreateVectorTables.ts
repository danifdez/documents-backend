...
export class CreateVectorTables1757668140320 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Agent working-folder files: single parent (indexed_files) → FK + CASCADE.
    await queryRunner.query(`
        ...
        `);
    await queryRunner.query(
      `CREATE INDEX "IDX_indexed_file_chunks_file_id" ON "indexed_file_chunks" ("indexed_file_id")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_indexed_file_chunks_owner_tag" ON "indexed_file_chunks" ("owner_tag")`,
    );
    await queryRunner.query(
